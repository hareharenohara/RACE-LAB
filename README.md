...
3. 一次選考では全候補の外部評価、単勝オッズ、人気順位、ウマークス荒度を比較し、Geminiが勝負する3レースと予算比率を決めます。
4. 発走約20分前に最新の出走情報と実オッズを再取得し、Geminiが全馬順位、確率、券種、組み合わせ、金額、見送り理由を決定します。
5. 開始資金は10万円、通常時の日次損失上限は開始残高の3.5%、購入は100円単位です。目標到達後はラチェット式ロックを守りながらATTACKへ移行します。
6. 複数点購入は可能ですが、最低オッズによる全着順パターンで的中時の回収割れが生じる場合はAIへ再調整を求めます。
7. 発走後の未確定レースを10分間隔で確認し、確定後に収支を反映します。
