...
): Record<Strategy, ValidSelection[]> {
  if (!value || typeof value !== "object") {
    throw new Error("SELECTION_NOT_OBJECT");
  }
  const root = value as Record<string, unknown>;
  const result = {} as Record<Strategy, ValidSelection[]>;
  for (const strategy of STRATEGIES) {
    if (!Array.isArray(root[strategy])) {
      throw new Error(`SELECTION_${strategy}_NOT_ARRAY`);
    }
    const items = (root[strategy] as Record<string, unknown>[]).map((item) => ({
    ...
    ) throw new Error(`SELECTION_${strategy}_FIELDS_INVALID`);
...
export function validatePredictions(
  ...
  if (!value || typeof value !== "object") {
    throw new Error("PREDICTION_NOT_OBJECT");
  }
  const root = value as Record<string, unknown>;
  if (root.strategy !== strategy || !Array.isArray(root.predictions)) {
    throw new Error("PREDICTION_SHAPE_INVALID");
  }
  ...
  for (const raw of root.predictions as Record<string, unknown>[]) {
    ...
    if (!validRaceIds.has(raceId)) throw new Error("PREDICTION_RACE_INVALID");
    if (seen.has(raceId)) throw new Error("PREDICTION_RACE_DUPLICATE");
    seen.add(raceId);
    if (!["BET", "SKIP"].includes(String(raw.action))) {
      throw new Error("PREDICTION_ACTION_INVALID");
    }
    const confidence = Number(raw.confidence);
    if (!Number.isInteger(confidence) || confidence < 0 || confidence > 100) {
      throw new Error("PREDICTION_CONFIDENCE_INVALID");
    }
    const bets = raw.bets;
    if (!Array.isArray(bets)) throw new Error("PREDICTION_BETS_INVALID");
    if (raw.action === "SKIP" && bets.length) throw new Error("SKIP_WITH_BETS");
    if (raw.action === "BET" && !bets.length) {
      throw new Error("BET_WITHOUT_BETS");
    }
    for (const bet of bets as Record<string, unknown>[]) {
      if (!BET_TYPES.includes(bet.type as BetType)) {
        throw new Error("BET_TYPE_INVALID");
      }
      const stake = Number(bet.stake);
      if (!Number.isInteger(stake) || stake <= 0 || stake % 100 !== 0) {
        throw new Error("BET_STAKE_INVALID");
      }
      if (
        !Array.isArray(bet.horses) ||
        bet.horses.length !== horseCount[bet.type as BetType] ||
        new Set(bet.horses.map(Number)).size !== bet.horses.length ||
        bet.horses.some((n) => !horseNumbers.get(raceId)?.has(Number(n)))
      ) throw new Error("BET_HORSES_INVALID");
      if (
        !String(bet.reason ?? "").trim() ||
        !String(bet.stake_reason ?? "").trim()
      ...
      items: {
        ...
        properties: {
          ...
          reference_pick: {
            ...
            required: ["horse_number", "bet_type", "horses", "reason"],
          },
          bets: {
            type: "array",
            items: {
        ...
        required: ["race_id", "action", "confidence", "reason", "bets"],
      },
    },
  },
  required: ["strategy", "predictions"],
};
