create or replace function public.finalize_prediction_decision(
...
begin
  ...
  loop
    insert into public.market_odds_snapshots(
      ...
      array(select jsonb_array_elements_text(v_bet->'horses'))::smallint[],
      (v_bet->>'odds')::numeric,nullif(v_bet->>'odds_max','')::numeric,
    ...
    v_bet_id := public.create_reserved_paper_bet(
      ...
      array(select jsonb_array_elements_text(v_bet->'horses'))::smallint[],
      (v_bet->>'stake')::integer,v_snapshot_id,(v_bet->>'odds')::numeric,
      null,null,null,v_bet->>'reason',v_bet->>'stake_reason'
    );

    insert into public.bet_decisions(
      prediction_id,bet_id,race_id,strategy,bet_type,combination,
      proposed_stake,final_stake,odds,minimum_expected_value,
      minimum_data_quality,decision,reason_code,reason_detail
    ) values (
      v_prediction_id,v_bet_id,p_race_id,p_strategy,v_bet->>'bet_type',
      array(select jsonb_array_elements_text(v_bet->'horses'))::smallint[],
      (v_bet->>'stake')::integer,(v_bet->>'stake')::integer,
      (v_bet->>'odds')::numeric,0,0,'purchased','AI_APPROVED',
      coalesce(nullif(v_bet->>'reason',''),'AI approved purchase')
    );
  end loop;
